...
class FutureBureau {
  ...
  addFutureBureauUser(user) {
    ...
    logger.enter(method);
    if (this.users[user]) {
      throw new Error(util.format('FutureBureau already exists in user %s', user));
    }
    this.users[user] = 1;
    logger.debug('%s - Result: %s', this.users, this.users[user]);
    logger.exit(method);
  }
  ...
  static async recordBet(targetBureau, opts) {
