...
class FutureBureau {
  ...
  static async recordBet(stub, targetBureau, opts) {
    const method = 'recordBet';
    try {
      logger.enter(method);
      logger.debug('%s - Expend %s amount of token %s, opts: %j', method, opts.amount, opts.tokenName, opts);
      const futureBureau = this.FROM_JSON(targetBureau);
      const options = this.FROM_JSON(opts);
      futureBureau.addHistory(opts.from, opts.to, opts.tokenName, opts.chooseOption, opts.amount, opts.timestamp);
      futureBureau.addFutureBureauUser(options.from);
      switch (options.chooseOption) {
        case 'option1':
          futureBureau.count1 += options.amount;
          break;
        case 'option2':
          futureBureau.count2 += options.amount;
          break;
        case 'option3':
          futureBureau.count3 += options.amount;
          break;
        case 'option4':
          futureBureau.count4 += options.amount;
          break;
        case 'option5':
          futureBureau.count5 += options.amount;
          break;
        default:
