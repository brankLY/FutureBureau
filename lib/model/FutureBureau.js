...
class FutureBureau {
  ...
  static async Create(stub, options) {
    ...
    logger.debug('%s - FutureBureaus are %s in Create', method, futureBureaus.bureaus);
    try {
  ...
  static async settle(stub, sReq, user) {
    const method = 'settle';
    try {
      logger.enter(method);
      ...
      let currentUser = await user.judge(stub, sReq);
      ...
      currentUser = await user.Get(stub);
      let len = 0;
      ...
      let i = 0;
      i = 0;
      logger.debug('transferTokenRequest currentUser is %s', currentUser.id);
      ...
      await Bureau.transferBack(stub, i, currentUser, len, futureBureau, rate, sReq.result, flag, token.gasPercentage, token.gasMin);
      logger.debug('%s - Successfully settled in bc, response: %s', method, currentUser.toString());
      return currentUser;
    } catch (e) {
      throw e;
    }
