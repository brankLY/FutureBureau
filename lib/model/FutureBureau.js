...
class FutureBureau {
  ...
  static async Create(stub, options) {
    ...
    await this.Save(stub, futureBureaus);
  ...
  static async Save(stub, futureBureau) {
    ...
    try {
      ...
      const recordtest = await stub.putState(key, Buffer.from(serializedFutureBureau));
      logger.debug('%s - test record %s', recordtest.toString('utf8'));
    } catch (e) {
