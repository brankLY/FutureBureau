...
class FutureBureau {
  ...
  static async getAll(stub) {
    ...
    // TODO: perform complex query to check if futureBureau with this name exists
    logger.enter(method);
    let record = (await stub.getState('FutureBureauRecord')).toString('utf8');
    if (!record) {
  ...
  static async Create(stub, options) {
    ...
    futureBureaus.history.push(futureBureau.name);
    logger.debug('%s - FutureBureaus are %s in Create', method, futureBureaus.history);
    await stub.putState('FutureBureauRecord', Buffer.from(futureBureaus.toString()));
    logger.debug('%s - FutureBureaus are %s in Create', method, futureBureaus.toString());
