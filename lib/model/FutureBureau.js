...
class FutureBureau {
  ...
  static async getAll(stub) {
    ...
    // TODO: perform complex query to check if futureBureau with this name exists
    logger.enter(method);
    const response = (await stub.getState('FutureBureauRecord')).toString('utf8');
    logger.debug('%s - FutureBureaus are %j', method, response);
    logger.exit(method);
    return response;
  }
  /**
  ...
  static async Create(stub, options) {
    ...
    logger.debug('%s - FutureBureaus are %j in Create', method, FutureBureauRecord);
