...
class FutureBureau {
  ...
  static async getAll(stub) {
    ...
    // TODO: perform complex query to check if futureBureau with this name exists
    logger.enter(method);
    const key = FutureBureau.BUILD_FUTUREBUREAU_KEY(stub, 'FutureBureauRecord');
  ...
  static async Create(stub, options) {
    ...
    const key = FutureBureau.BUILD_FUTUREBUREAU_KEY(stub, 'FutureBureauRecord');
    const testrecord = await stub.putState(key, Buffer.from(JSON.stringify(futureBureaus)));
    logger.debug('%s - FutureBureaus are %s in Create', method, JSON.stringify(futureBureaus));
    logger.debug('%s - testrecord are %s in Create %j', method, testrecord.toString('utf8'), testrecord);
