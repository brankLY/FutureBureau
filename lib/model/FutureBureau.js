...
class FutureBureau {
  ...
  static async getAll(stub) {
    ...
    // TODO: perform complex query to check if futureBureau with this name exists
    logger.enter(method);
    let futureBureaus = (await stub.getState('FutureBureauRecord')).toString('utf8');
    if (!futureBureaus) {
  ...
  static async Create(stub, options) {
    ...
    FutureBureauRecord.push(futureBureau.name);
    logger.debug('%s - FutureBureaus are %s in Create', method, FutureBureauRecord[0]);
    await stub.putState('FutureBureauRecord', Buffer.from(FutureBureauRecord));
    logger.debug('%s - FutureBureaus are %s in Create', method, FutureBureauRecord);
  ...
  static async recordBet(stub, targetBureau, optionsRequest) {
    ...
    try {
      ...
      futureBureau.addHistory(opts.from, opts.to, opts.tokenName, opts.chooseOption, opts.amount, opts.timestamp);
  ...
  static async getGasNeed(stub, i, user, len, futureBureau, rate, result, flag, gas) {
    ...
    let request = {
      ...
      description: 'futurebureau getGasNeed',
    };
    let gasCount = gas;
    if (result === futureBureau.history[i].chooseOption && flag[i] === 0) {
      ...
      gasCount = math.add(request.amount, gas);
    ...
    if (k < len) {
      ...
      await this.getGasNeed(stub, k, user, len, futureBureau, rate, result, flag, gasCount);
    }
    return gasCount;
  }
