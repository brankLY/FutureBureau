...
class FutureBureau {
  ...
  static async Create(stub, options) {
    ...
    let newFutureBureaus = (await stub.getState('FutureBureauRecord')).toString('utf8');
    logger.debug('- FutureBureaus are %j in Create %s', futureBureaus, futureBureaus.bureauNames);
    logger.debug('FutureBureaus in chain is %j', newFutureBureaus);
    if (!newFutureBureaus) {
      newFutureBureaus = futureBureaus;
      newFutureBureaus.bureauNames.push(options.name);
      await stub.putState('FutureBureauRecord', Buffer.from(JSON.stringify(newFutureBureaus)));
    } else {
      logger.debug('- FutureBureaus are %j in Create ', newFutureBureaus);
      newFutureBureaus = JSON.parse(newFutureBureaus);
      newFutureBureaus.bureauNames.push(options.name);
      await stub.putState('FutureBureauRecord', Buffer.from(JSON.stringify(newFutureBureaus)));
    }
    logger.debug('%s - FutureBureaus are %s ', method, newFutureBureaus.bureauNames);
    logger.debug('%s - FutureBureaus are %s in Create ', method, JSON.stringify(newFutureBureaus));
  ...
  static FROM_JSON(obj) {
    ...
    futureBureau.chineseName = obj.chineseName;
  ...
  static async NEW_EMPTY_FUTUREBUREAU(stub, name) {
    ...
    try {
      ...
      let futureBureau = new FutureBureau();
      futureBureau.name = futureBureauInfo.name;
      futureBureau.chineseName = futureBureauInfo.chineseName;
      futureBureau.creator = futureBureauInfo.creator;
      ...
      futureBureau.result = 'Undecided';
      futureBureau.history = [];
      logger.exit(method);
      futureBureau = futureBureau.toJSON();
      return futureBureau;
    } catch (e) {
      throw e;
    }
