...
class FutureBureau {
  ...
  static async Create(stub, options) {
    ...
    logger.debug('%s - Check contract %j', method, options);
    const request = JSON.parse(options);
    logger.debug('%s - Check contractAccountId %s', method, request.contractAccountId);
    await stub.putState('futurebureau', Buffer.from(options.contractAccountId));
    // create a new Contract Account at Earth
    const params = ['account.create', options.contractAccountId, 'Dapp_futurebureau'];
    await stub.invokeChaincode(EARTH_CHAINCODE_ID, params);
    const futureBureauObj = {
      id: options.contractAccountId,
      name: options.name,
      creator: id,
