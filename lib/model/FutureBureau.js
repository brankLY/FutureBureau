...
class FutureBureau {
  ...
  static async getAll(stub) {
    ...
    // TODO: perform complex query to check if futureBureau with this name exists
    logger.enter(method);
    const key = FutureBureau.BUILD_FUTUREBUREAU_KEY(stub, 'record');
    const ttest = (await stub.getState('justtest')).toString('utf8');
    logger.debug('%s test is %s', ttest);
    let record = (await stub.getState(key)).toString('utf8');
    logger.debug('%s - record: %s', record);
    if (!record) {
  ...
  static async Create(stub, options) {
    ...
    futureBureaus.history.push(options.name);
    logger.debug('%s - FutureBureaus are %s in Create', method, futureBureaus.history);
    await stub.putState('FutureBureauRecord', Buffer.from(JSON.stringify(futureBureaus)));
    const tesst = await stub.putState('justtest', Buffer.from('test'));
    logger.debug('%s - FutureBureaus are %s in Create test is %s', method, JSON.stringify(futureBureaus), tesst.toString('utf8'));
