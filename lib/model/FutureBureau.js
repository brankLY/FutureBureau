...
class FutureBureau {
  ...
  static async recordBet(stub, targetBureau, optionsRequest) {
    ...
    const options = Object.assign({}, targetBureau);
    try {
      logger.enter(method);
      logger.debug('%s - Expend %s amount of token %s, opts: %j', method, opts.amount, opts.tokenName, options.futureBureauName);
      const futureBureau = FutureBureau.Get(stub, options);
