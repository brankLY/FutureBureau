...
class FutureBureau {
  ...
  static async Create(stub, options) {
    ...
    return futureBureau.Save(stub, futureBureau.toJSON());
  }

  static async Save(stub, futureBureauObj) {
    const futureBureau = FutureBureau.FROM_JSON(stub, futureBureauObj);
  ...
  async save() {
    const method = 'save';
    try {
      const key = this.BUILD_FUTUREBUREAU_KEY(this.stub, this.name);
      await stub.putState(key, Buffer.from(this));
    } catch (e) {
  ...
  static async recordBet(stub, targetBureau, optionsRequest) {
    ...
    try {
      logger.enter(method);
      logger.debug('%s - Expend %s amount %s, targetBureau: %s', method, opts.amount, opts.tokenName, targetBureau.futureBureauName);
      const futureBureau = FutureBureau.Get(stub, targetBureau);
  ...
  static async Get(stub, options) {
    ...
    logger.exit(method);
    return FutureBureau.FROM_JSON(futureBureau);
  }
