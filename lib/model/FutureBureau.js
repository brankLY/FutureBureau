...
class FutureBureau {
  ...
  addHistory(from, to, tokenName, chooseOption, chooseOdds, amount, timestamp) {
    this.history.push({
      from,
      to,
      tokenName,
  ...
  static async Create(stub, options) {
    ...
    futureBureau.addHistory(id, id, " " , "option+1.6", 0, 0, stub.getTxTimestamp().seconds.low * 1000);
  ...
  static FROM_JSON(stub,obj) {
    ...
    futureBureau.odds1 = obj.odds1;
    futureBureau.odds2 = obj.odds2;
    futureBureau.odds3 = obj.odds3;
    futureBureau.history = obj.history || [];
    futureBureau.result = obj.result;
    
    // Object.entries(obj).forEach(([id, user]) => {
    //   futureBureau.users[id] = User.FROM_JSON(user);
    // });
    logger.exit(method);
    return futureBureau;
  }
  ...
  static async NEW_EMPTY_FutureBureau(stub, name) {
    ...
    const id = identityService.getName();
      ...
      futureBureau.odds1 = futureBureauInfo.odds1;
      futureBureau.odds2 = futureBureauInfo.odds2;
      futureBureau.odds3 = futureBureauInfo.odds3;
      futureBureau.result = " ";
  ...
  static CHECK_CREATE_OPTIONS(options) {
    ...
    if (!options.odds2) {
      throw new Error(util.format('%j is not a valid CreateFutureBureauOption Object, Missing Required property %s', options, 'odds2'));
    }
    if (!options.odds3) {
    ...
    if (!TypeChecker.checkUnsignedFloat(options.odds1)) {
      throw new Error(util.format('%j is not a valid float for CreateFutureBureauOption.decimals', options.odds1));
    }
    if (!TypeChecker.checkUnsignedFloat(options.odds2)) {
      throw new Error(util.format('%j is not a valid float for CreateFutureBureauOption.decimals', options.odds2));
    }
    if (!TypeChecker.checkUnsignedFloat(options.odds3)) {
      throw new Error(util.format('%j is not a valid float for CreateFutureBureauOption.decimals', options.odds3));
    }
  }
