...
class Bureau {
  ...
  addNewFutureBureau(futureBureau) {
    ...
    this.futureBureaus[futureBureau.name] = futureBureau;
    logger.debug('method %j', this.futureBureaus[futureBureau.name]);
    logger.exit(method);
  }
   ...
   */
  async recordBet(targetBureau, opts) {
    const method = 'recordBet';
    try {
      logger.enter(method);
      logger.debug('%s - Expend %s amount of token %s, opts: %j', method, opts.amount, opts.tokenName, opts);
      logger.debug('+++++++++++++++++ %j', this.futurebureaus[targetBureau.name]);
      logger.debug('++++++++yyy+++++++++ %s', targetBureau.name);
      const futureBureau = this.futurebureaus[this.futurebureaus[targetBureau.toJSON(targetBureau).name]];
