...
class Bureau {
  ...
  async addFutureBureauHistory(stub, name, history) {
    ...
    if (!this.futureBureaus[name]) {
      ...
      let futureBureau = await FutureBureau.NEW_EMPTY_FUTUREBUREAU(stub, name);
      futureBureau = FutureBureau.FROM_JSON(futureBureau);
      this.futureBureaus[name] = futureBureau;
      logger.debug('this.futureBureaus %j', this.futureBureaus[name]);
    }
    await this.futureBureaus[name].history.push(history);
    logger.debug('futureBureaus %j', this.futureBureaus[name]);
    logger.debug('user is now %j', this);
    logger.exit(method);
    return this.save();
  }
  /**
