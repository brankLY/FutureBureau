...
class Bureau {
  ...
  toJSON() {
    const res = {};
    Object.entries(this.futureBureau).forEach(([name, futureBureau]) => {
   ...
   * @param {FutureBureau} token
   */
  async recordBet(futureBureauName, tokenName, amount, opts) {
    ...
    try {
      ...
      if (!futureBureau) {
        throw new Error(util.format('%s - There is not enough futureBureau %s in wallet', method, tokenName));
      }
      futureBureau.addHistory(opts.from, opts.to, tokenName, opts.chooseOption, amount, this.stub.getTxTimestamp().seconds.low * 1000);
      logger.exit(method);
    } catch (e) {
