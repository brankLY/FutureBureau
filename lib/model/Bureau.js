...
class Bureau {
   ...
   */
  addNewFutureBureau(futureBureau, history) {
    const method = 'addNewFutureBureau';
    logger.enter(method);
    const opts = Object.assign({}, history);
    this.futureBureaus[futureBureau.name] = futureBureau;
    this.futureBureaus[futureBureau.name].addHistory(opts.from, opts.to, opts.tokenName, opts.chooseOption, opts.amount, opts.timestamp);
