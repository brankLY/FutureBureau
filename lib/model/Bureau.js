...
class Bureau {
   ...
   */
  async addNewFutureBureau(stub, targetBureau, history) {
    const method = 'addNewFutureBureau';
    logger.enter(method);
    const opts = Object.assign({}, history);
    let futureBureau = await FutureBureau.Get(stub, targetBureau);
    futureBureau = FutureBureau.FROM_JSON(futureBureau);
    this.futureBureaus[targetBureau.name] = futureBureau;
    this.futureBureaus[targetBureau.name].addHistory(opts.from, opts.to, opts.tokenName, opts.chooseOption, opts.amount, opts.timestamp);
    logger.debug('%s is %j', futureBureau.name, futureBureau.toJSON());
    logger.exit(method);
  }
