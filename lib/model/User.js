...
class User {
  ...
  static async Get(stub, userId) {
    ...
    logger.debug('%s - get key %s', method, user);
    logger.debug('%s - get key %s', method, account.toString('utf8'));
    const decoder = new StringDecoder('utf8');
    logger.debug('%s +++++++++', decoder.write(account));
    account = JSON.stringify(account);
    logger.debug('%s - get key %s', method, account);
    ...
    logger.debug(' - get 66666 user %s', user);
