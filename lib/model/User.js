...
class User {
  ...
  static async Get(stub, userId) {
    ...
    logger.debug('%s - get key1 %s', method, user);
    // const iconv = new Iconv('UTF-8', 'ASCII//TRANSLIT//IGNORE');
    // const test = iconv.convert(account);
    const decodedText = iconv.decode(account, 'gbk');
    logger.debug('%s - get key1 %s', method, decodedText);
    account = JSON.stringify(account);
    logger.debug('%s - get key2 %s', method, account);
    account = JSON.parse(account);
    logger.debug('%j - get key3 %s', account, account);
    let copy = account.payload.buffer;
    copy = Buffer.from(copy);
    // let test = copy.split('@');
    // logger.debug(' - %s get 9999 key %s', test[0], test);
    // copy = JSON.parse(copy);
    logger.debug(' - %s get 77777 key %s', copy[0], copy);
    copy = copy.toString('utf8');
    logger.debug(' - %s get 66666 key %s', copy[0], copy);
    if (!user) {
