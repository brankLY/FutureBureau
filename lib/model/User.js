...
class User {
  ...
  async createNewFutureBureau(opts) {
    ...
    try {
      createFutureBureauOptions.odds1 = parseFloat(createFutureBureauOptions.odds1, 10);
      createFutureBureauOptions.odds2 = parseFloat(createFutureBureauOptions.odds2, 10);
      createFutureBureauOptions.odds3 = parseFloat(createFutureBureauOptions.odds3, 10);
    } catch (e) {
      logger.error('%s - Can not parse opts.odds1:%s or opts.odds2:%s or opts.odds3:%s to double, error: %o', method, opts.odds1, opts.odds2, opts.odds3, e);
  ...
  async betTransfer(stub, opts) {
    ...
    try {
      ...
      const { 
        target: targetId, futureBureauName, tokenName, chooseOption, chooseOdds, amount 
      } = opts;
      // check if target user exists
      let target = await User.Get(this.stub, targetId);
