...
class User {
  ...
  static FROM_JSON(stub, obj) {
    ...
    logger.debug('%s +++++++user.bureau++++++++++ %j', user.bureau, user.bureau);
    logger.debug('%s +++++++user.wallet+++++++++ %j', user.wallet, user.wallet);
    return user;
  }
  ...
  static async Get(stub, userId) {
    ...
    let temp = account.payload.toString('utf8');
    logger.debug('- toString(account) in user %s', temp);
    temp = JSON.parse(temp);
    logger.debug('%j - JSON.parse in user %s', temp, temp);
    if (!user) {
      logger.error('%s - Can not find User %s', method, id);
      throw new Error(util.format('User %s does not exist', id));
    }
    user = JSON.parse(user);
    user.canCreateNewFutureBureau = temp.canCreateNewFutureBureau;
    user.wallet = temp.wallet;
  ...
  static async Create(stub, options, init) {
    ...
    let account = await stub.invokeChaincode('earth', ['user.create', options.id, options.name]);
    account = account.payload.toString('utf8');
    account = JSON.parse(account);
    logger.debug('%s - account %j : %j', method, account, account.wallet);
    const user = {
      id: options.id,
      name: options.name,
      role: options.role,
