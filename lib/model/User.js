...
class User {
  ...
  async betTransfer(opts) {
    ...
    try {
      ...
      const { target: targetId, futureBureauName, amount } = opts;
      // check if target user exists
      let target = await User.Get(this.stub, targetId);
      ...
      this.wallet.expend(opts.tokenName, amount, history);
      await target.wallet.earn(opts.tokenName, amount, history);
