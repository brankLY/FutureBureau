...
class User {
  ...
  static async createNewFutureBureau(stub, opts) {
    const method = 'createNewFutureBureau';
    logger.enter(method);
    const user = User.FROM_JSON(stub, opts);
    if (!user.canCreateNewFutureBureau) {
    ...
    try {
      const futureBureau = await FutureBureau.Create(stub, createFutureBureauOptions);
      user.bureau.addNewFutureBureau(futureBureau);
      user.canCreateNewFutureBureau = false;
      logger.exit(method);
      return user.save();
    } catch (e) {
