...
class User {
  ...
  async betTransfer(opts) {
    ...
    try {
      ...
      await this.bureau.addFutureBureauHistory(this.stub, targetBureau.name, history);
      logger.debug('+++++++user++++++++++ %j', this);
  ...
  static async judge(stub, judgeOptions) {
    const method = 'Judge';
    logger.enter(method);
    ...
    currentUser.bureau.addNewFutureBureau(currentFutureBureau);
    logger.debug('Judge %s', judgeOptions.futureBureauName);
    logger.debug('result is %s', judgeOptions.result);
    logger.debug('currentUser is %j', currentUser.toJSON());
    logger.exit(method);
  ...
  static MIX_USER(stub, user, account) {
    logger.debug(' - wallet %s', account.wallet);
    const newuser = {
      id: user.id,
      name: user.name,
      role: account.role,
