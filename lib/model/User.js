...
class User {
  ...
  async transfer(opts) {
    ...
    try {
      ...
      logger.debug('%s - transfer to target %s', method, targetId);
  ...
  async betTransfer(opts) {
    ...
    try {
      ...
      this.wallet.expend(tokenName, opts.amount, history);
      await target.wallet.earn(tokenName, opts.amount, history);
  ...
  static async Get(stub, userId) {
    ...
    const account = await stub.invokeChaincode('earth', ['user.query']);
    // logger.debug('%s - Query account %j', account, account);
    // const test = JSON.stringify(account);
    // logger.debug('%s - JSON.stringify(account) %j', test, test);
    // const test1 = JSON.parse(test);
    // logger.debug('%s - JSON.parse(account) %j', test1, test1);
    // const test2 = Buffer.from(test1.payload.buffer);
    // logger.debug('%s - Buffer.fron(account) %j', test2, test2);
    // const test3 = test2.toString();
    // const temp = test3.split('@');
    // logger.debug('%s - toString(account) %j', temp, temp);
    // const temp1 = temp[0].substring(10, temp[0].length - 1);
    // logger.debug('%s - toString(account) %s', temp1, temp1.length);
    // const temp2 = JSON.parse(temp1);
    let temp2 = account.payload.buffer.toString('utf8');
    logger.debug('%j - toString(account) in user %s', temp2, temp2);
    temp2 = JSON.parse(temp2);
    logger.debug('%j - JSON.parse in user %s', temp2, temp2);
    if (!user) {
      logger.error('%s - Can not find User %s', method, id);
      throw new Error(util.format('User %s does not exist', id));
    }
    user = JSON.parse(user);
    user.canCreateNewFutureBureau = temp2.canCreateNewFutureBureau;
    user.wallet = temp2.wallet;
