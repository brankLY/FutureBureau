...
class User {
  ...
  const account = await stub.invokeChaincode('earth', ['create', options.id, options.name, options.role, false, false , Wallet.NEW_EMPTY_WALLET(stub).toJSON(), Bureau.NEW_EMPTY_BUREAU(stub).toJSON()], 'mychannel');
  logger.debug('%j - account %s exists:', account);
