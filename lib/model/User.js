...
class User {
  ...
  async betTransfer(opts) {
    ...
    try {
      ...
      const { target: targetId, tokenName, futureBureauName } = opts;
      // check if target user exists
      let target = await User.Get(this.stub, targetId);
      const targetBureau = await FutureBureau.Get(this.stub, opts);
      ...
      await this.bureau.recordBet(futureBureauName, history);
      // save and return
      logger.debug('Save Target user');
      target = await target.save();
      logger.debug(target.toString());
