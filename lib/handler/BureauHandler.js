  ...
  */
  static async initContractAccount(stub, params) {
    ...
    try {
      ...
      await stub.putState('futurebureau', Buffer.from(Request.contractAccountId));
  ...
  static async getAll() {
    ...
    try {
      ...
      const futureBureaus = FutureBureau.getAll();
      logger.debug('%s - Successfully get User from bc, response: %s', method, futureBureaus.toString());
      logger.exit(method);
