...
class UserHandler {
  ...
  static async create(stub, params) {
    ...
    try {
      ...
      const account = await stub.invokeChaincode('earth', ['user.create', params[0], params[1]]);
      logger.debug('- Create new account with options %j ,string is %s', account.toJSON(), account.toBuffer());
