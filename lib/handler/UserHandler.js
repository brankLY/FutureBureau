...
class UserHandler {
  ...
  static async settle(stub, params) {
    ...
    try {
      ...
      logger.debug('history`s amount is %j', futureBureau.history.amount);
      logger.debug('rate is %s ', rate);
      const testTransferTokenRequest = {
        target: futureBureau.history.from,
        tokenName: futureBureau.history.tokenName,
        amount: math.multiply(futureBureau.history.amount, rate),
      };
