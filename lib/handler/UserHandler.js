  ...
  static async getOneFutureBureau(stub, params) {
    ...
    try {
      ...
      if (params.length !== 1) {
        ...
        return Response(false, 'Query FutureBureau requires params of length 1');
      }
      logger.debug('%s - Query FutureBureau', method);
      let queryBureauRequest;
      try {
        queryBureauRequest = JSON.parse(params[0]);
      } catch (e) {
        return Response(false, 'Can not parse request params[0] to query bureau Object');
      }
