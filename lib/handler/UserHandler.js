...
class UserHandler {
  ...
  static async transfer(stub, params) {
    ...
    try {
      ...
      if (params.length !== 1) {
        logger.error('%s - Transfer Token requires params of length 1, [target, tokenName, amount] , found %s, %j', method, params.length, params);
      ...
      logger.debug('%s - Transfer Options:%j', transferTokenRequest);
