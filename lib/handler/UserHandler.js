...
class UserHandler {
  ...
  static async betTransfer(stub, params) {
    ...
    try {
      ...
      await stub.invokeChaincode(EARTH_CHAINCODE_ID, ['account.transfer', params[0]]);
  ...
  static async update(stub, params) {
    ...
    try {
      ...
      await stub.invokeChaincode(EARTH_CHAINCODE_ID, ['account.update', params[0], params[1]]);
      logger.debug('%s - Update User %s with newValue %j', method, params[0], params[1]);
  ...
  static async createToken(stub, params) {
    ...
    try {
      ...
      const account = await stub.invokeChaincode(EARTH_CHAINCODE_ID, ['account.createToken', params[0]]);
      const user = await User.Get(stub);
  ...
  static async transfer(stub, params) {
    ...
    try {
      ...
      const account = await stub.invokeChaincode(EARTH_CHAINCODE_ID, ['account.transfer', params[0]]);
      logger.debug('- invokeChaincode Options:%j', account);
      const user = await User.Get(stub);
  ...
  static async transferBack(stub, i, user, len, futureBureau, rate, result, flag) {
    let request = {
    ...
    if (result === futureBureau.history[i].chooseOption && flag[i] === 0) {
      ...
      await stub.invokeChaincode(EARTH_CHAINCODE_ID, ['account.transfer', request]);
    ...
    if (k < len) {
      ...
      await this.transferBack(stub, k, user, len, futureBureau, rate, result, flag);
    }
  }
  ...
  static async settle(stub, params) {
    ...
    try {
      ...
      await this.transferBack(stub, i, user, len, futureBureau, rate, settleRequest.result, flag);
