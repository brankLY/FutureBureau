...
class UserHandler {
  ...
  static async transferBack(i, user, len, futureBureau, rate, result) {
    ...
    if (result === futureBureau.history[i].chooseOption) {
    ...
    if (k < len) {
      await this.transferBack(k, user, len, futureBureau, rate, result);
    }
  }
  ...
  static async settle(stub, params) {
    ...
    try {
      ...
      logger.debug('rate is %s ', rate);
      await this.transferBack(i, user, len, futureBureau, rate, settleRequest.result);
