...
class UserHandler {
  ...
  static async withdraw(stub, params) {
    ...
    try {
      ...
      const account = await stub.invokeChaincode(EARTH_CHAINCODE_ID, args);
      logger.debug('- invokeChaincode Options:%j', account);
      logger.debug('%s - Successfully transferred Token in bc, response: %s', method, account.payload.toString('utf8'));
      return Response(true, JSON.stringify(account.payload.toString('utf8')));
    } catch (e) {
      logger.error('%s - Error: %o', method, e);
      return Response(false, e.message);
    }
