...
class UserHandler {
  ...
  static async transferBack(user, len, futureBureau, rate) {
    for (let i = 0; i < len; math.add(i, 1)) {
      let h = futureBureau.history[i];
  ...
  static async settle(stub, params) {
    ...
    try {
      ...
      logger.debug('rate is %s ', rate);
      const len = futureBureau.history.length;
      logger.debug('%d - Get history length', len);
      this.transferBack(user, len, futureBureau, rate);
