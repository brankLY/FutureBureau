  ...
  static async getOne(stub, params) {
    ...
    try {
      ...
      if (params.length !== 0) {
        ...
        return Response(false, 'Query User requires params of length 0');
      }
      ...
      logger.exit(method);
      let [ response ]:any  = await stub.invokeChaincode('earth', ['user.query']);
      response = response.toString('utf8');
      logger.debug('%s - Query account', response);
