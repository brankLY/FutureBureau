...
class UserHandler {
  ...
  static async transferBack(i, user, len, futureBureau, rate, result) {
    ...
    logger.debug('futureBureau.history.chooseOption is %j', futureBureau.history.chooseOption);
    if (result === futureBureau.history.chooseOption) {
    ...
    k = math.add(k, 1);
    if (k < len) {
      this.transferBack(k, user, len, transferTokenRequest);
    }
  }
  ...
  static async settle(stub, params) {
    ...
    try {
      ...
      logger.debug('rate is %s ', rate);
      this.transferBack(i, user, len, futureBureau, rate, settleRequest.result);
