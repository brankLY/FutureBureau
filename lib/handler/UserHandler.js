...
class UserHandler {
  ...
  static async settle(stub, params) {
    ...
    try {
      ...
      logger.debug('history`s amount is %s', futureBureau.history.amount);
      logger.debug('rate is %s ', rate);
      const testTransferTokenRequest = {
        target: futureBureau.history[0].from,
        tokenName: futureBureau.history[0].tokenName,
        amount: math.multiply(futureBureau.history[0].amount, rate),
      };
      User.CHECK_TRANSFER_OPTIONS(testTransferTokenRequest);
      await user.transfer(testTransferTokenRequest);
      logger.debug(' transfer test1');
      user.transfer(testTransferTokenRequest);
      logger.debug(' transfer test2');
      user.transfer(testTransferTokenRequest);
      logger.debug(' transfer test3');
      futureBureau.history.forEach((h) => {
        if (h.chooseOption === futureBureau.result) {
          const transferTokenRequest = {
            target: h.from,
            tokenName: h.tokenName,
            amount: math.multiply(h.amount, rate),
          };
