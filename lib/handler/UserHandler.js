...
class UserHandler {
  ...
  static async getOneFutureBureau(stub, params) {
    ...
    try {
      ...
      logger.debug('%s - Successfully get FutureBureau from bc, response: %s', method, user.toString());
      logger.exit(method);
  ...
  static async settle(stub, params) {
    ...
    try {
      ...
      let futureBureau = await FutureBureau.Get(stub);

      futureBureau.users.forEach((id) => {
        let futureBureauUser = futureBureau.users[id];
        futureBureauUser.bureau.futureBureaus.forEach((id) => {
          let settleFutureBureau = futureBureauUser.bureau.futureBureaus[id];
          if (settleFutureBureau == futureBureau){
            if (settleFutureBureau.history.chooseOdds == params[1]){
              let newParams;
              newParams[0] = settleFutureBureau.history.from;
              newParams[1] = settleFutureBureau.name;
              newParams[2] = settleFutureBureau.history.tokenName;
              newParams[3] = settleFutureBureau.history.chooseOption;
              newParams[4] = settleFutureBureau.history.chooseOdds
              newParams[5] = settleFutureBureau.history.amount * settleFutureBureau.history.chooseOdds;
              let transferTokenRequest = JSON.parse(newParams);
