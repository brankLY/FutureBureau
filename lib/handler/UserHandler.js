    ...
    try {
      ...
      futureBureau.history.forEach((h) => {
        if (h.chooseOption === futureBureau.result) {
          ...
          logger.debug('transferTokenRequest is %j', JSON.parse(transferTokenRequest));
          user = User.Get(stub);
          logger.debug('transferTokenRequest user is %j', user.toJSON());
          user = user.transfer(JSON.parse(transferTokenRequest));
        }
      });
