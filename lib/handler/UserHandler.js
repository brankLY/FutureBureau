...
class UserHandler {
  ...
  static async betTransfer(stub, params) {
    ...
    try {
      ...
      await stub.invokeChaincode('earth', ['user.transfer', params[0]]);
  ...
  static async update(stub, params) {
    ...
    try {
      ...
      await stub.invokeChaincode('earth', ['user.update', params[0], params[1]]);
  ...
  static async transferBack(stub, i, user, len, futureBureau, rate, result, flag) {
    ...
    if (result === futureBureau.history[i].chooseOption && flag[i] === 0) {
      ...
      logger.debug('%s------show inline request---------- %s', request, request[0]);
      await stub.invokeChaincode('earth', ['user.transfer', request]);
