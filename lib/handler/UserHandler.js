...
class UserHandler {
  ...
  static async settle(stub, params) {
    ...
    try {
      ...
      const user = await User.Get(stub);
      await user.Judge(stub, settleRequest);
      const futureBureau = await FutureBureau.Get(stub, settleRequest);
      let count;
      if (settleRequest.result === futureBureau.option1) {
        count = futureBureau.count1;
      }
      if (settleRequest.result === futureBureau.option2) {
        count = futureBureau.count2;
      }
      if (settleRequest.result === futureBureau.option3) {
        count = futureBureau.count3;
      }
      if (settleRequest.result === futureBureau.option4) {
        count = futureBureau.count4;
      }
      if (settleRequest.result === futureBureau.option5) {
        count = futureBureau.count5;
      }
      const rate = (futureBureau.count1 + futureBureau.count2 + futureBureau.count3 + futureBureau.count4 + futureBureau.count5) / count;
      futureBureau.history.forEach((h) => {
        if (h.chooseOption === futureBureau.result) {
          const transferTokenRequest = {
            target: h.from,
            futureBureauName: futureBureau.name,
            tokenName: h.tokenName,
            chooseOption: h.chooseOption,
            amount: h.amount * rate,
          };
          user.transfer(stub, transferTokenRequest);
        }
      });
