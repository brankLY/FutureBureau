...
class UserHandler {
  ...
  static async create(stub, params) {
    ...
    try {
      ...
      await stub.invokeChaincode('earth', ['user.create', params[0], params[1]]);
  ...
  static async getOne(stub, params) {
    ...
    try {
      ...
      logger.debug('%s - toString(account) %s', temp2, temp2.length);
  ...
  static async createToken(stub, params) {
    ...
    try {
      ...
      const account = await stub.invokeChaincode('earth', ['user.createToken', params[0]]);
      const user = await User.Get(stub);
