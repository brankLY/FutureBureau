...
class UserHandler {
  ...
  static async betTransfer(stub, params) {
    ...
    try {
      ...
      const checkTime = await this.checkTime(targetBureau);
      logger.debug('- checkTime in betTransfer :%s', checkTime);
      if (!checkTime) {
        logger.error('FutureBureau %s already end', targetBureau.name);
        return Response(false, 'FutureBureau already end');
      }
  ...
  //     let result = true;
  //     if (endTime <= now) {
  //       logger.debug('FutureBureau %s already end', options.name);
  //       result = false;
  //     }
  //     logger.debug('%s - result is  %s', method, result);
  //     logger.exit(method);
  //     return result;
  //   } catch (e) {
  //     logger.error('%s - Error: %o', method, e);
